...
public class ApplicationDecisionAlreadyMadeException extends RuntimeException {
    public ApplicationDecisionAlreadyMadeException(ApplicationDetails applicationDetails){
        super(String.format("Application %d approval was set to %b on %s", applicationDetails.getId(), applicationDetails.isApproved(), applicationDetails.getDecisionDate().atZone(ZoneId.systemDefault())));
    }
}
